...
namespace icub
{
	namespace robot_model
	{
		...
		public:
			...
			virtual void setPose(Matrix &q)
			{
				mRoot->setPose(q, T_ROOT);

				gravDirty = selfDirty = true;
			}

			virtual void setPoseCalcJ(Matrix &q)
			{
				mRoot->setPoseCalcJ(q, T_ROOT);
			...
			virtual void calcInterference(Matrix &distance);

			virtual const Matrix& calcInterferenceAndJ(Matrix &distance);
